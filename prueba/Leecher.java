...
public class Leecher implements Runnable {
    ...
    private Integer obtenerIndiceFaltante() { // metodo para ver si hay algun bloque faltante,
        ...
        if(faltantes.isEmpty()) return null; //No falta ningun indice del archivo
    ...
    public void run() {
        ...
        while (!soySeeder) {
            ...
            if (key == null) {
                semaforos.getSemaforoPrinter().acquireUninterruptibly();
                System.out.println();
                System.out.println("Proceso "+this.id+" con archivo al 100%:");
                mostrar_archivo();
                System.out.println();
                System.out.println("MATANDO PID "+this.id+"...");
                semaforos.getSemaforoPrinter().release();
                this.soySeeder = true;
                break;
            }

            semaforos.getSemaforoTracker().acquireUninterruptibly();
            if (tracker.consultarBloque(key, this.id) == null) { // si no hay ningun leecher o seeder que tenga el dato, vamos al servidor
                semaforos.getSemaforoTracker().release();

                semaforos.getSemaforoServer().acquireUninterruptibly();
                ...
                semaforos.getSemaforoServer().release();

                if(contenido_bloque == null) continue;
                else{
                    ...
                    //Actualizar tracker
                    semaforos.getSemaforoTracker().acquireUninterruptibly();
                    this.tracker.actualizarTracker(key, (Integer)this.id);
                    semaforos.getSemaforoTracker().release();
                }
                ...
                semaforos.getSemaforoById(consultar_a.getId());
                //System.out.println("PID "+this.id + " solicita BLOQUE: "+key+" a " + "PID "+consultar_a.getId());
                String contenido_bloque = consultar_a.enviarDato(key);
                semaforos.getSemaforoById(consultar_a.getId()).release();
